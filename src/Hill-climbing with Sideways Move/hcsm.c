...
void hcsm(Cube *cube)
{
    ...
    //? inisialisasi linear_cube. INI SUPAYA NANTI NGE SWAP GAMPANG, GA PERLU NESTED LOOP
    ...
    while (improved && h_current < TOTAL_EDGES)
    {
        ...
        for (u1 = 0; u1 < TOTAL_VALUES - 1; u1++)
        {
            for (u2 = u1 + 1; u2 < TOTAL_VALUES; u2++)
            {
                ...
                //? kalo heuristicsny sama dan total_sideways < limit (i.e. 100 / bebas), update h_best DAN total_sideways
