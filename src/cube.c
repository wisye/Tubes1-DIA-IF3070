...
bool check_cube(Cube *cube) {
	...
	if(space_diag1_sum != MAGIC_NUMBER || space_diag2_sum != MAGIC_NUMBER || space_diag3_sum != MAGIC_NUMBER || space_diag4_sum != MAGIC_NUMBER) return false;
...
int calculate_fitness(Cube *cube) {
	...
	for(int i = 0; i < SIZE; i++){
		for(int j = 0; j < SIZE; j++){
			...
			if(row_sum == MAGIC_NUMBER) fitness++;
		}
	}
	...
	for(int i = 0; i < SIZE; i++){
		for(int k = 0; k < SIZE; k++){
			...
			if(col_sum == MAGIC_NUMBER) fitness++;
		}
	}
	...
	for(int j = 0; j < SIZE; j++){
		for(int k = 0; k < SIZE; k++){
			...
			if(depth_sum == MAGIC_NUMBER) fitness++;
		}
	}
	...
	for(int i = 0; i < SIZE; i++){
		...
		if(diag1_sum == MAGIC_NUMBER) fitness++;
		if(diag2_sum == MAGIC_NUMBER) fitness++;
	}
	...
	if(space_diag1_sum == MAGIC_NUMBER) fitness++;
	if(space_diag2_sum == MAGIC_NUMBER) fitness++;
	if(space_diag3_sum == MAGIC_NUMBER) fitness++;
	if(space_diag4_sum == MAGIC_NUMBER) fitness++;
